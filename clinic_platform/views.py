...
def uploads(request):
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        if file_type == 'variants':
            ...
            variants2network('uploads/S5527_aligned_to_h37rv_RG_sorted_filtered_ann_unique.vcf', genome_graph,
                             'processed')
            variants2network('uploads/S507_aligned_to_h37rv_RG_sorted_filtered_ann_unique.vcf', genome_graph,
                             'processed')
...
def gene_list_input(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'gene_list_input.html',
                      {'form': form_class, 'bam_list': clean_bam_list, 'gff_list': clean_gff_list})
...
def gene_view(request, gene_id):
    ...
    return render(request, 'gene_view.html',
                  {'JSON_data': data, 'table': expression_table_data, 'gene_info': json_res_info, 'gene_name': gene_id})
...
def coverage_summary_old(request):
    if request.method == 'POST':
        ...
        return render(request, 'coverage_summary.html',
                      {'JSON_data': q_gene, 'exon_cot_row': exon_sample_row, 'cot_matrix': cot_matrix,
                       'view_info': view_info})
        ...
        return render(request, 'coverage_summary_select.html',
                      {'gene_list': gene_list, 'track_name': loaded_custom_tracks['custom_annotation']})
...
def coverage_summary_sample(request):
    ...
    if request.method == 'POST':
            ...
            if isinstance(gene_cov_result, str):
                ...
                for a_trans in gene_cov_result.keys():
                    ...
                    for a_sample in sample_list:
                        ...
                        sample_cot_row = [{'cot': gene_cov_result[a_trans][a_sample]['percentage_over_cov_thres'],
                                           'avc': gene_cov_result[a_trans][a_sample]['average_coverage'],
                                           'cot_perc': gene_cov_result[a_trans][a_sample][
                                               'percentage_over_cov_thres_perc']}]
        ...
        return render(request, 'coverage_summary_samples.html',
                      {'gene_list': gene_list, 'sample_list': sample_list, 'cot_matrix': table_matrix,
                       'view_info': view_info, 'error_list': error_list})
...
def gene_file_to_db(request):
    if request.method == 'POST':
        ...
        return render(request, 'custom_gene_set_upload.html',
                      {'anno_file_list': custom_anno_files, 'bam_list': clean_bam_list, 'vcf_list': clean_vcf_list})
...
def panel_detail_view(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'panel_details.html',
                      {'panel_list': panel_list, 'panel_name': selected_panel, 'gene_info': gene_status_list,
                       'bam_list': bam_list})
